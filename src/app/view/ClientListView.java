...
public class ClientListView extends JPanel {
    ...
    private JPanel createButtonPanel() {
        ...
        JButton seeDetails = createActionButton("Ver Detalles", "app/icon/svg/details-icon.svg");

        // Asignar acciones a los botones
        setupNewButtonAction(newButton);
        setupEditButtonAction(editButton);
        setupDeleteButtonAction(deleteButton);
        setupSeeDetailsButtonAction(seeDetails);

        buttonPanel.add(newButton);
        buttonPanel.add(editButton);
        buttonPanel.add(deleteButton);
        buttonPanel.add(seeDetails);
    ...
    private void setupSeeDetailsButtonAction(JButton button) {
        button.addActionListener(e -> {
            int selectedRow = table.getSelectedRow();
            if (selectedRow != -1) {
                String id = table.getValueAt(selectedRow, 0).toString();
                ...
                Notifications.getInstance().show(Notifications.Type.INFO, "Seleccione un cliente para ver sus detalles.");
            }
        });
    }
}
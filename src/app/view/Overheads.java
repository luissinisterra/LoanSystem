...
public class Overheads extends JPanel {
    ...
    public void llenarTabla() {
        try{
            ...
            for (int i = 0; i < controller.getByUserID(user.getId(), this.user).size(); i++) {
                model.addRow(new Object[]{
                        controller.getByUserID(user.getId(), this.user).get(i).getOverheadType(),
                        controller.getByUserID(user.getId(), this.user).get(i).getAmmount(),
                        controller.getByUserID(user.getId(), this.user).get(i).getOverheadDate(),
                });
            }
    ...
    public void setTotal () {
        try{
            List<OverheadResponseDTO> overheads = controller.getByUserID(user.getId(), this.user);
            double total = 0;
    ...
    private void editOverhead(){
        ...
        if (id == null) {
            ...
            OverheadResponseDTO overhead = controller.getByID(id, this.user);
    ...
    private void removeOverhead(){
        ...
        if (id == null) {
            ...
            controller.removeGasto(id, this.user);
            llenarTabla();
            setTotal();
    ...
    private void detailsOverhead(){
        ...
        if  (id == null) {
            ...
            JOptionPane.showMessageDialog(null, "ID: " + id + "\n" +
                    "Tipo: " + controller.getByID(id, this.user).getOverheadType() + "\n" +
                    "Valor: " + controller.getByID(id, this.user).getAmmount() + "\n" +
                    "Detalles: " + controller.getByID(id, this.user).getOverheadDescription() + "\n" +
                    "Fecha: " + controller.getByID(id, this.user).getOverheadDate());
        }
    }
